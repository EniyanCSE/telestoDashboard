document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateGraph(selectedWell) {
    Plotly.d3.csv("well_level_history_matching_and_prediction.csv", function (csvData) {
      ...
      data.forEach(d => {
        d.MONTHS = +d["Months"];
        d["Oil_rate(klpd)"] = +d["Oil_rate(klpd)"];
        d["Oil_rate_actual(klpd)"] = +d["Oil_rate_actual(klpd)"];
      });

      // Create a trace for the predicted oil rate
      const trace1 = {
        x: data.map(d => d.MONTHS),
        y: data.map(d => d["Oil_rate(klpd)"]),
        type: 'scatter',
        mode: 'lines',
        name: 'Prediction',
        line: { color: 'blue' },
      ...
      const trace2 = {
        x: data.map(d => d.MONTHS),
        y: data.map(d => d["Oil_rate_actual(klpd)"]),
        type: 'scatter',
        mode: 'lines',
        name: 'Actual',
        line: { color: 'red' },
  ...
  function getLayout(selectedWell) {
    return {
      title: `${selectedWell} - Oil Rate Over Time`,
      xaxis: {
        title: 'Months',
        type: 'linear',
      },
