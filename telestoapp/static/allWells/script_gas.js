...
d3.csv("field_level_history_matching_and_prediction.csv").then(data => {
  data.forEach(d => {
    d["MONTHS"] = +d["MONTHS"];
    d["Gas_pred(mscum d)"] = +d["Gas_pred(mscum d)"];
    d["Gas_actual(mscum d)"] = +d["Gas_actual(mscum d)"];
  });

  const filteredData = data.filter(d => !isNaN(d["MONTHS"]) && !isNaN(d["Gas_pred(mscum d)"]) && !isNaN(d["Gas_actual(mscum d)"]));

  // Create a trace for the predicted Gas rate
  const trace1 = {
    x: filteredData.map(d => d["MONTHS"]),
    y: filteredData.map(d => d["Gas_pred(mscum d)"]),
    type: 'scatter',
    mode: 'lines',
    name: 'Predicted Gas Rate',
    line: { color: 'blue' },
  ...
  const trace2 = {
    x: filteredData.map(d => d["MONTHS"]),
    y: filteredData.map(d => d["Gas_actual(mscum d)"]),
    type: 'scatter',
    mode: 'lines',
